  ...
  my.Wrapper = function(endpoint, options) { 
    ...
    this._convertFilter = function(filter) {
      ...
      if (filter.type === 'term') {
        ...
        out.range.from = filter.from;
        out.range.to = filter.to;
      }
      return out;
    },
      ...
      _.each(queryObj.filters, function(filter) {
        ...
        if (filter.type === 'term') {
          results = _.filter(results, function(doc) {
            return (doc[filter.field].match(new RegExp(filter.term, "gi")));
          });
        }
      });
      return results;
        ...
        results = _.filter(results, function(rawdoc) {
          ...
          _.each(terms, function(term) {
            ...
            _.each(self.fields, function(field) {
              ...
              // TODO regexes?
              foundmatch = foundmatch || (value.toLowerCase().match(new RegExp(term.toLowerCase(), "gi")));
...
my.Grid = Backbone.View.extend({
  ...
  onMenuClick: function(e) {
    ...
    var actions = {
      ...
      sortAsc: function() { self.setColumnSort('asc'); replaceSemantics();},
      sortDesc: function() { self.setColumnSort('desc'); replaceSemantics(); },
  ...
  template: ' \
    ...
    <table class="recline-grid table-striped table-condensed" cellspacing="0" border="0" width="100%"> \
...
//    $dataSidebar.append(facetViewer.el);
  },
...
my.Fields = Backbone.View.extend({
  ...
  template: ' \
    ...
    <h3 class="sidebar">Fields <a href="#" class="js-show-hide">+</a></h3> \
    {{#fields}} \
        ...
        <div id="collapse{{id}}" class="accordion-body collapse in"> \
          <div class="accordion-inner"> \
            ...
            <div class="facet-summary" data-facet="{{id}}"> \
              ...
              {{#terms}} \
                <li class="facet-item" data-value="{{term}}"><a class="facet-choice js-facet-filter"><span class="term">{{term}}</span> <span class="count">[{{count}}]</span></a></li> \
              {{/terms}} \
              </ul> \
            </div> \
            {{/facets}} \
  ...
  onFacetFilter: function(e) {
    var $target= $(e.target);
    var fieldId = $target.closest('.facet-summary').attr('data-facet');
    var value = $target.closest('.facet-item').attr('data-value');
  //  alert(fieldId + ":" + value);
   /* var self = this;
    e.preventDefault();
    var filters = self.model.queryState.get('filters');
    filters[0].term = value;
      ...
      <form class="form-stacked js-add" style="display: none;"> \
        <fieldset> \
          ...
          <select class="filterType"> \
            ...
            <option value="range">Range (Numerical Data)</option> \
          </select> \
          <label>Field</label> \
          <select class="fields"> \
            {{#fields}} \
    ...
    range: ' \
      <div class="filter-{{type}} filter"> \
        <fieldset> \
          ...
          <label class="control-label" for="">End</label> \
   ...
   var self = this;
    ...
    _.each($form.find('input'), function(input) {
      ...
      if (filterType === 'term') {
        ...
        filters[filterIndex].from = parseFloat(value);
        filters[filterIndex].to = parseFloat(value);
      }
    });
    self.model.queryState.set({filters: filters});
    self.model.queryState.trigger('change');
