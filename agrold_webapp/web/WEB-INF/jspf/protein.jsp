...
var protein = {
    ...
    html:'<nav id="proteinPage" class="nav nav-tabs flex-column flex-sm-row advs">\
                ...
                <div id="publicationContainer">\
                    <span id="publicationPageBtns" class="pageNavBtns"><a class="flex-sm-fill text-sm-center nav-link active o-publicationResult" href="javascript:void(0)" id="publication"> Publication </a></span>\
                </div>\
            </nav>\
...
BIND(REPLACE(str(?entity), \'^.*(#|/)\', \"\") AS ?Id) \
    ...
    searchGenesEncodingProtein:function(page) {
        ...
        var tthis=this;
        swagger.apis.gene.getGenesEncodingProteins({_format: ".sparql-json", proteinId: ModalContext.id, _pageSize: pageSize, _page: page},
    ...
    searchQtlsAssociatedWithProtein:function(page) {
        ...
        var tthis=this;
        swagger.apis.qtl.getQtlsAssociatedWithProteinId({_format: ".sparql-json", proteinId: ModalContext.id, _pageSize: pageSize, _page: page},
        {responseContentType: 'application/json'}, function (data) {
            var sparqljson = data.data;
            var resultId = type + "Result";
            displayResult(resultId, sparqljson);
    ...
    searchOntologyTermsAssociatedWithProtein:function(proteinId, page) {
        ...
        displayHoldMessage(type + "Result");
        var tthis=this;
        swagger.apis.ontologies.getOntoTermsAssociatedWithProtein({_format: ".sparql-json", proteinId: ModalContext.id, _pageSize: pageSize, _page: page},
        {responseContentType: 'application/json'}, function (data) {
